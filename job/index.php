<?php
$mysqli = new mysqli("localhost", "root", "", "job_tracker");
...
<body class="min-h-screen p-4 font-sans">
  ...
  <?php
    ...
    $colors = [
      'saved' => 'hover:bg-orange-100',
      'applied' => 'hover:bg-yellow-100',
      'offered' => 'hover:bg-green-100',
      'rejected' => 'hover:bg-red-100',
      'interview' => 'hover:bg-blue-100',
      'accepted' => 'hover:bg-green-200'
    ];
    foreach ($labels as $label):
  ?>
  <div class="text-center bg-white p-4 rounded-xl shadow transition duration-300 ease-in-out <?= $colors[$label] ?> hover:scale-105 hover:shadow-xl cursor-pointer">
  ...
  <?php endforeach; ?>
      ...
      <div class="grid md:grid-cols-3 gap-6 mb-6">
        ...
        <div class="p-6 bg-white rounded-xl shadow-md">
          ...
          <button onclick="toggleOverviewModal()" class="mt-4 btn-orange py-1 px-3 rounded hover:opacity-90 text-sm">
            View Details
          </button>
        </div>
      </div>
      ...
      <div class="grid md:grid-cols-3 gap-6">
        ...
        <div class="p-6 bg-white rounded-xl shadow-md">
          ...
          <?php if ($saved_jobs_result && $saved_jobs_result->num_rows > 0): ?>
            <table class="w-full text-left text-dark text-sm">
              <thead><tr><th>Job Title</th><th>Company</th><th>Status</th></tr></thead>
              <tbody>
  ...
  <!-- Modal -->
  <div id="overviewModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-[9999] transition-all duration-300 ease-in-out">
    <div id="overviewModalContent" class="bg-white p-6 rounded-xl max-w-sm w-full shadow-2xl transform scale-95 opacity-0 transition-all duration-300 ease-in-out">
      <h3 class="text-lg font-semibold mb-4 text-dark">Application Status Breakdown</h3>
      <ul class="text-sm text-gray-700 space-y-2">
        <li>📂 Saved: <?= (int)$overview['saved'] ?></li>
        <li>📨 Applied: <?= (int)$overview['applied'] ?></li>
        <li>🏁 Offered: <?= (int)$overview['offered'] ?></li>
        <li>❌ Rejected: <?= (int)$overview['rejected'] ?></li>
        <li>🗓️ Interview: <?= (int)$overview['interview'] ?></li>
        <li>✅ Accepted: <?= (int)$overview['accepted'] ?></li>
      </ul>
      <button onclick="toggleOverviewModal()" class="mt-6 w-full bg-gray-300 hover:bg-gray-400 text-black py-1 rounded">
        Close
      </button>
    </div>
  </div>
